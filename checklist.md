...
- [x] main 배포 후 live smoke로 소개팅 `terse_low_context` 4턴 전부 NEEDS를 확인한다.

## AMERICAN_LEARNER 질문 의도 충족 평가 보정

- [x] 덕메 Q2의 `어쩌다 입덕했어?`에 대한 `영상 봤어.` 단답 사례를 RED 테스트로 고정한다.
- [x] 입덕 계기 질문에 맥락 부족 답변이 오면 NEEDS로 전환한다.
- [x] `AMERICAN_LEARNER` turn feedback 프롬프트에 입덕 계기 단답 기준을 명시한다.
- [x] focused 테스트, 전체 unittest, compileall, diff check를 실행한다.
- [x] main 배포 후 live smoke로 덕메 Q2 품질 변화를 확인한다.
...
- [x] focused 테스트, 전체 unittest, compileall, diff check를 실행한다.
- [x] main 배포 후 live smoke로 덕메 Q1 품질 변화를 확인한다.

## AMERICAN_LEARNER 반복 품질 스모크

- [x] 최소 3회 BE live smoke를 동일 조건으로 실행한다.
- [x] 각 run의 계약 게이트 위반 수를 JSON 원본 기준으로 집계한다.
- [x] 품질상 애매한 사례를 input/output과 판단 쟁점으로 분리한다.
- [x] 명확한 회귀가 있으면 최소 수정하고 로컬 테스트, 배포, 재스모크로 확인한다.
- [x] 최종 결과와 사용자 판단 필요 항목을 정리한다.

## AMERICAN_LEARNER BE live smoke 계약 위반 보정

- [x] BE live smoke 원본에서 next-question 언어 혼합과 GOOD feedbackDetail 한국어 출력을 확인한다.
- [x] root cause를 `context-notes.md`에 기록한다.
- [x] 미국인 모드 next-question이 영어 고정 질문을 `aiQuestion`에 섞으면 한국어 고정 질문으로 복구하는 RED 테스트를 추가한다.
