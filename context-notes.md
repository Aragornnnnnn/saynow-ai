...
- 2026-05-24 시나리오 성공 또는 실패 여부는 피드백 생성 결과가 아니라 백엔드 세션 결과다. AI 피드백 API는 백엔드가 확정한 `scenarioResult`를 입력으로 받아 총평과 발화별 피드백이 세션 결과와 충돌하지 않게 사용한다.
- `scenarioResult` 값은 우선 `SUCCESS`, `FAILURE` 두 상태로 제한한다. SSE 이벤트 순서는 그대로 `summary`, `turnFeedback`, `done`을 유지하고, 별도 result 이벤트는 만들지 않는다.
- `scenarioResult=FAILURE`일 때 모델이 성공권 점수나 성공 총평을 반환해도 AI 서버가 `comprehensionScore`를 59 이하로 낮추고 실패 총평으로 보정한다. 이 보정은 기본 피드백과 SSE summary 생성 경로에 모두 적용한다.
