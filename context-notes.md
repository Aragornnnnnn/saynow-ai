...
- 2026-06-06 develop 코드의 모델 라우팅은 속도 우선 `next-question=gpt-4o-mini`, 품질 우선 `turn-feedback/session-feedback=gpt-5.4-mini`로 나눈다. fallback은 OpenAI provider 안에서만 `gpt-4o-mini`로 한 번 재시도하며, primary 호출 예외와 JSON 파싱 실패를 대상으로 한다. Upstage 같은 비 OpenAI provider는 기존 provider 모델을 그대로 쓴다.
